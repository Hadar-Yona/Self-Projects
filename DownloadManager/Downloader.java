    ...
    public void run()
    {
        ...
        for (String range : rangesForDownload) 
        {
            try 
            {
                ...
                int reader;
                int newRangeStart;
                chunk.rangeStart = Integer.parseInt(range.split("-")[0]);       // zero as limit
                
                while((reader = inStream.readNBytes(chunk.chunkContentByteArray, 0, chunk.chunkContentByteArray.length)) != 0)  // zero as the start offset in chunkConteentByteArray
                {                
                    ...
                    chunk = new FileChunk(newRangeStart, 0);          //will be changed later
                }
