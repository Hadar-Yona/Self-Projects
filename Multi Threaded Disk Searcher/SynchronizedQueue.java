...
public class SynchronizedQueue<T> {
	...
	public T dequeue() {
		synchronized (this) {
			if (buffer == null) {
				...
				} else {                                  // no more producers
					return null;
				}
			}
			int i = 0;
			T element = buffer[0];                    // removes the first element in the queue
			while (i < this.size) {
				buffer[i] = buffer[i + 1];
				i++;
			}
	...
	public void enqueue(T item) {
		synchronized (this)
		{
			while (this.getCapacity() == this.size)                     // the queue is full
			{
				try {
					wait();
				} catch (InterruptedException e) {
			...
			buffer[size] = item;                                        // put a new element in the end of the queue
			this.size++;
			if (this.size == 1) {                                       // before the enqueue the queue was empty
				notifyAll();
			}
		}
	}
